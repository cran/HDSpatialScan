...
##'
##' @description This function returns the significant clusters and their associated p-value for the MNP
##'
...
##'
##'
MNP <- function(data, sites_coord = NULL, system = NULL, mini = 1, maxi = nrow(sites_coord)/2, type_minimaxi = "sites/indiv", mini_post = NULL, maxi_post = NULL, type_minimaxi_post = "sites/indiv", sites_areas = NULL, MC = 999, typeI = 0.05, nbCPU = 1){
  ...
  if(class(data)[1]=="matrix"){
    ...
    if(nbCPU <=1 | num_cores<2){
      results <- lapply(1:MC, function(i) multi_WMW(generation_signif[[i]], matrix_clusters))
    }else{
      if(num_cores < nbCPU){
        nbCPU <- num_cores
      }
      ...
      if(Sys.info()["sysname"] == "Windows"){
        ...
        results <- parLapply(cl, 1:MC, function(i) multi_WMW(generation_signif[[i]], matrix_clusters))
        stopCluster(cl)

      }else{
        results <- mclapply(1:MC, function(i) multi_WMW(generation_signif[[i]], matrix_clusters), mc.cores = nbCPU)
      }
...
##'
##' @description This function returns the significant clusters and their associated p-value for the MG.
##'
##' @param data matrix. Matrix of the data, the rows correspond to the sites (or the individuals if the observations are by individuals and not by sites) and each column represents a variable.
...
##' data("multi_data")
##' coords <- coordinates(map_sites)
##' res_mg <- MG(data=multi_data, sites_coord = coords, system = "WGS84",
##' mini = 1, maxi = nrow(coords)/2)}
##' \dontshow{
##' library(sp)
##' data("map_sites")
...
##' indices <- which(map_sites$NAME_3 == "Lille")
##' coords <- coordinates(map_sites[indices,])
##' res_mg <- MG(data=multi_data[indices,], sites_coord = coords,
...
##'
##'
MG <- function(data, sites_coord = NULL, system = NULL, mini = 1, maxi = nrow(sites_coord)/2, type_minimaxi = "sites/indiv", mini_post = NULL, maxi_post = NULL, type_minimaxi_post = "sites/indiv", sites_areas = NULL, MC = 999, typeI = 0.05, nbCPU = 1){
  ...
  if(class(data)[1]=="matrix"){
    ...
    if(nbCPU <=1 | num_cores <2){
      results <- lapply(1:MC, function(i) multi_gaussian(generation_signif[[i]], matrix_clusters))
    }else{
      ...
      if(Sys.info()["sysname"] == "Windows"){
        cl <- makeCluster(nbCPU)
        results <- parLapply(cl, 1:MC, function(i) multi_gaussian(generation_signif[[i]], matrix_clusters))
        stopCluster(cl)

      }else{
        results <- mclapply(1:MC, function(i) multi_gaussian(generation_signif[[i]], matrix_clusters), mc.cores = nbCPU)
      }
